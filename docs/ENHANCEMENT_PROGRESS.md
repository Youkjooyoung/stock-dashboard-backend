...
- 시작일: 2026-04-16
- 범위: **A-3 (코드 품질 증명)** + **A-4 (자동화 역량)** + **B-A2 (Redis 캐시)** + **B-A1 (Prometheus + Grafana 관측성)**
...
## B-A1. Prometheus + Grafana 관측성 도입 ✅

### 배경

운영 환경에서 장애 징후(메모리 누수·HikariCP 고갈·응답 p95 급등·캐시 히트율 급락)를 **사후가 아닌 실시간 계측**으로 잡아내는 지표 스택이 없었음. Spring Boot Actuator + Micrometer + Prometheus + Grafana 조합은 업계 de-facto 구성이며, 면접 시 "도입해본 적 있다"로 설명 가능한 가장 낮은 비용/높은 가치 트랙.

### 구현

| 파일 | 상태 | 변경 내용 |
|------|------|----------|
| `stock-dashboard/pom.xml` | ✅ | `spring-boot-starter-actuator` + `io.micrometer:micrometer-registry-prometheus` 추가 (Spring Boot BOM 관리 버전 1.13.12 + Prometheus 1.x 클라이언트) |
| `stock-dashboard/src/main/resources/application.properties` | ✅ | `management.endpoints.web.exposure.include=*`, `management.endpoint.health.probes.enabled=true`, `management.metrics.tags.application=${spring.application.name}`, `management.prometheus.metrics.export.enabled=true` — 전체 지표에 `application="stock-dashboard"` 태그 자동 부여 |
| `stock-dashboard/src/main/java/com/stock/dashboard/SecurityConfig.java` | ✅ | `/actuator/health`, `/actuator/prometheus`, `/actuator/metrics/**`, `/actuator/info` permitAll — Prometheus가 인증 없이 스크레이핑 가능 |
| `infra/prometheus/prometheus.yml` | ✅ | `stock-dashboard-backend` job — `/actuator/prometheus` 스크레이핑, `scheme: https` + `tls_config.insecure_skip_verify: true` (자가서명 인증서 대응) |
| `infra/grafana/provisioning/datasources/prometheus.yml` | ✅ | Prometheus 데이터소스 자동 프로비저닝 |
| `infra/grafana/provisioning/dashboards/dashboards.yml` | ✅ | `/var/lib/grafana/dashboards` 폴더 프로비저닝 선언 |
| `infra/grafana/dashboards/spring-boot-overview.json` | ✅ | **6 패널 대시보드**: JVM Heap Used, CPU(process/system), HTTP request rate by status, HTTP p95 latency per URI, HikariCP 커넥션(Active/Idle/Pending), Cache hit/miss rate |
| `docker-compose.yml` | ✅ | `prometheus:latest`(9090), `grafana:latest`(3001) 서비스 추가 — 각각 `infra/prometheus/`, `infra/grafana/provisioning/` 볼륨 마운트, `depends_on: backend` |
| `stock-dashboard/src/test/java/com/stock/dashboard/ActuatorEndpointsTest.java` | ✅ | `@AutoConfigureObservability` + `@ActiveProfiles("test")` — health 200/UP, `/actuator/prometheus` 200 & `jvm_memory_used_bytes`·`application="stock-dashboard"` 포함, `http_server_requests_seconds` 메트릭 존재 검증 (**3 케이스**) |
| `stock-dashboard/src/test/resources/application-test.properties` | ✅ | 테스트용 Actuator 설정 — `spring.cache.type=simple`, exposure(health/info/metrics/prometheus), `management.health.db|redis|mail.enabled=false` (외부 의존성 미기동 환경 결정성 확보) |

### 디버깅 노트 — 왜 `PrometheusMetricsExportAutoConfiguration`이 뜨지 않았나?

Spring Boot 테스트 컨텍스트는 `ObservabilityContextCustomizerFactory$DisableObservabilityContextCustomizer`로 **기본적으로 모든 메트릭 익스포터를 비활성화**한다 (테스트를 빠르게 유지하기 위함). 초기 증상:
- `/actuator/health` → 500/503 (Redis 헬스체크 NPE, `db` HikariPool init 실패)
- `/actuator/prometheus` → 500 `NoResourceFoundException: No static resource actuator/prometheus.`
- Bean 스캔 시 `SimpleMetricsExportAutoConfiguration`만 존재, `prometheusMeterRegistry` 없음

해결: 테스트 클래스에 `@AutoConfigureObservability` 추가하여 관측성 재활성 + 테스트 프로파일에서 외부 의존성 헬스 인디케이터(`db/redis/mail`) 비활성화. 테스트가 "endpoint 와이어링" 검증에 집중하도록 의존성 최소화.

### 기대 효과

- **실시간 가시성**: 장애 징후를 로그 grep이 아닌 Grafana 대시보드 한 눈에 — p95 latency·히트율·힙 추이 즉시 포착
- **병목 추적**: HTTP URI별 p95로 어느 엔드포인트가 느려지는지 한 번에 식별
- **커넥션 풀 포화 경보**: HikariCP Active/Pending으로 DB 풀 고갈 조기 감지
- **캐시 효과 증명**: Redis 캐시 도입 전/후 hit ratio 수치로 성능 효과 정량 제시 — **포트폴리오 스토리텔링 강화**

### 운영 확장 포인트

- Prometheus/Grafana 엔드포인트는 VPC 내부망 또는 별도 관리 포트로 분리 권장 (현재 `exposure.include=*`는 로컬/개발 기준)
- Grafana 로그인: `admin/admin` 기본(환경변수 `GRAFANA_USER/PASSWORD`로 오버라이드)
- Alertmanager 연동 + Slack/이메일 알림은 차기 작업 후보
...
- [x] Actuator + Micrometer-Prometheus 의존성, `/actuator/prometheus` 노출, SecurityConfig permitAll
- [x] infra/prometheus + infra/grafana 프로비저닝 (6 패널 대시보드)
- [x] docker-compose에 prometheus + grafana 서비스
- [x] ActuatorEndpointsTest 3 케이스 (@AutoConfigureObservability 적용)

---

## 로컬 실행 결과 (2026-04-16)

- `./mvnw test` → **BUILD SUCCESS**, 79 tests: 72 passed, 7 skipped
  ...
  - **ActuatorEndpointsTest: 3 ✅ (health/prometheus/httpServerRequests 메트릭)**

...
포트폴리오 후속 고도화(품질 + 자동화) 작업 트래킹 문서.

- 시작일: 2026-04-16
- 범위: **A-3 (코드 품질 증명)** + **A-4 (자동화 역량)**
- 목표: 면접에서 보여줄 수 있는 **테스트 커버리지** + **CI/CD 파이프라인** 증거 확보
...
- `./mvnw test` → **BUILD SUCCESS**, 73 tests: 66 passed, 7 skipped
  - UserServiceTest: 21 ✅ (중첩 클래스 5개 포함)
  - JwtUtilTest: 9 ✅
  - InputValidatorTest: 35 ✅ (ParameterizedTest 확장 후)
  - StockDashboardApplicationTests: 1 ✅ (context load)
...
5. 차기 작업 후보 (사용자 선택):
   - **B급 A-1**: Grafana/Prometheus 관측성 (로그 지표)
   - **B급 A-2**: Redis 캐시 도입 (주식 시세 조회)
...
- 다음 세션: 로컬에서 `./mvnw test` 실행하여 테스트 초록불 확인 → GitHub Actions Secrets 등록 → 배포 파이프라인 실전 검증.

...
**Backend**
- Java 21 (LTS), Spring Boot 3.3.10
- MyBatis 3.0.3 (ORM)
- MySQL 8.0
...
| 2026-04-16 | **B-A2 Redis 캐시**: `spring-boot-starter-data-redis` 도입, `CacheConfig` 프로파일 분기(`spring.cache.type=simple`(matchIfMissing) → `ConcurrentMapCacheManager` / `=redis` → `RedisCacheManager`), 캐시별 TTL (`latestPrices` 10m, `allItems` 1h, `priceByTicker` 30m), `GenericJackson2JsonRedisSerializer` JSON 직렬화, `prefixCacheNameWith("stock-dashboard::")` 네임스페이스, `docker-compose.yml`에 `redis:7-alpine` 서비스 추가(256MB LRU + AOF + healthcheck, backend `depends_on: service_healthy`), `CacheConfigTest` 3 케이스(bean 주입 분기 검증) | 백엔드 |
| 2026-04-16 | **B-A1 관측성 스택**: `spring-boot-starter-actuator` + `micrometer-registry-prometheus` 도입, `/actuator/prometheus` 노출 + `application="stock-dashboard"` 태그 자동 부여, `SecurityConfig` permitAll (`/actuator/health`, `/actuator/prometheus`, `/actuator/metrics/**`, `/actuator/info`), `infra/prometheus/prometheus.yml` scrape 설정(HTTPS + insecure_skip_verify), `infra/grafana/provisioning` 데이터소스/대시보드 자동 등록, 6 패널 대시보드(`spring-boot-overview.json`: JVM Heap / CPU / HTTP req rate by status / HTTP p95 per URI / HikariCP 커넥션 / 캐시 hit·miss rate), `docker-compose.yml`에 `prometheus:latest`(9090) + `grafana:latest`(3001) 추가, `ActuatorEndpointsTest` 3 케이스(`@AutoConfigureObservability` 필수 — Spring Boot 테스트 컨텍스트가 기본적으로 메트릭 익스포터를 비활성화하는 이슈 해결) | 백엔드, 인프라 |
